    ...
    public long get() {
        ...
        if (as != null) {
            for (int i = 0; i < as.length; ++i) {
                if ((a = as[i]) != null)
                    result = function.applyAsLong(result, a.value); // ## rename when JDK8 syncs with lambda, applyAsLong
            }
        }
        return result;
    }
        ...
        if (as != null) {
            for (int i = 0; i < as.length; ++i) {
                if ((a = as[i]) != null) {
                    long v = a.value;
                    a.value = identity;
                    result = function.applyAsLong(result, v); // ## rename when JDK8 syncs with lambda, applyAsLong
                }
            }
        }
        return result;
